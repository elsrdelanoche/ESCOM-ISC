...
{
    int C[3][3]; // matriz para cofactores de matriz key[][]
    int A[3][3]; // matriz para el adjunto de la matriz C[][]
    ...
    {
        printf("El texto no puede ser decodificado, usa una matriz valida.\n");
        exit(1);
    }
...
int main()
{
    ...
    string text = "ACTBEFOREDAWNZZ";
 
    string c = encrypt(text, key);
    cout << "El texto codificado es:  " << c;
 
    int inv[3][3];
    inverseMatrix(key, inv);
 
    cout << "\nEl texto decodificado es " << decrypt(c, inv);
