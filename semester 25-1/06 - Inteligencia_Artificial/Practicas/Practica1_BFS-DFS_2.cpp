...
void dfs(int u, int x) {
    ...
    if (u == x) {
        cout << "Me encontraste nodo" <<u<<" en el nivel: "<<vis[u]<<endl<<" En "<<ttime<<" pasos,";  // Si se encuentra el nodo buscado, imprime un mensaje.
    ...
    for (auto it : g[u])  // Recorre los vecinos del nodo actual.
        if (!vis[it]){
            vis[it]=vis[u]+1;  // Si un vecino no ha sido visitado, llama recursivamente a DFS.
            dfs(it, x);
        }
}
...
void bfs(int u, int x) {
    ...
    while (!q.empty()) {
        ...
        if (v == x) {
            cout << "Me encontraste nodo" <<v<<" en el nivel: "<<vis[u]<<endl<<" En "<<ttime<<" pasos,"; // Si encuentra el nodo buscado, imprime el mensaje.
        ...
        for (auto it : g[v]) {  // Recorre los vecinos del nodo actual.
            if (!vis[it]) {
                q.push(it);
                vis[it] = vis[v]+1;  // Marca los vecinos como visitados y los añade a la cola.
            }
        }
    }
}
