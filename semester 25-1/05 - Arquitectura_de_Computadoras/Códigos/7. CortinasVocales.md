...
; Sección para almacenar la animación en memoria de programa

cortinas_data:
    .hword  0x0088, 0x0086, 0x00F9, 0x0040, 0x0041, 0x0000; LEDs extremos encendidos, termina en 0x0000

    
; Bucle principal
main_loop:
    CALL    CierreCortinas   ; Llamar a la animación automáticamente
    BRA     main_loop        ; Repetir la animación en bucle

; Subrutina para animar el cierre de cortinas
CierreCortinas:
    MOV     #tblpage(cortinas_data), W0    ; Obtener página de tabla
    MOV     W0, TBLPAG                     ; Cargar TBLPAG con la página
    MOV     #tbloffset(cortinas_data), W1  ; Obtener offset inicial

loop_cortinas:
    TBLRDL [W1++], W4                    ; Leer un paso de la animación (8 bits)
    CP0     W4                             ; ¿Es el final de la animación? (0x00)?
    BRA     Z, fin_cortinas                ; Si es 0, salir de la animación
    MOV     W4, PORTB                      ; Enviar el valor a los LEDs
    CALL    retardo                    ; Esperar el retardo fijo
    BRA     loop_cortinas                  ; Repetir con el siguiente paso
