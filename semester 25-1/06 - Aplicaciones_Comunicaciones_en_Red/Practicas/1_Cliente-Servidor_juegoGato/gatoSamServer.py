...
def servidor():
    ...
    PORT = 54321  # Puerto fijo para simplificar la conexión
    buffer_size = 1024
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as TCPServerSocket:
        ...
        while True:
            ...
            with Client_conn:
                ...
                # Preguntar si el cliente quiere jugar otra partida
                Client_conn.sendall("¿Quieres jugar otra partida? (si/no)".encode('utf-8'))
                respuesta = Client_conn.recv(buffer_size).decode('utf-8').strip().lower()
                if respuesta != "si":
                    print("El cliente ha decidido terminar la conexión.")
                    break
