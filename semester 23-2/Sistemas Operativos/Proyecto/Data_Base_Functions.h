...
char**   Get_Attribute_Values(char* DB_Text,unsigned int* Attributes_Amount,unsigned int* Char_ptr,unsigned int* Values_amount){
    ...
    *Values_amount = 0;
    while (DB_Text[*Char_ptr] != ',' && DB_Text[*Char_ptr] != ']')
    {
        ...
        *Char_ptr += 1;
        if (DB_Text[*Char_ptr] == '&')
        {
            Array_Buffer = Append_String(Array_Buffer,Attributes_Amount,Buffer);
            *Char_ptr += 1;
            *Values_amount+=1;
            Buffer = NULL;
        }
...
unsigned int* Add_Extra_Sub_Attribute(unsigned int* value_amount, unsigned int size_required){
    unsigned int* New_Array;
    if (value_amount == NULL)
    {
        New_Array = (unsigned int*)calloc(sizeof(unsigned int),1);
        if (New_Array == NULL){printf("Error Allocating Extra Value Array!!\n");return NULL;}
        return New_Array;
    }else{
        New_Array = (unsigned int*)realloc(value_amount,sizeof(unsigned int)*size_required);
        if (New_Array == NULL){printf("Error Resizing Extra Value Array!!\n");return NULL;}
        return New_Array;
    }
...
Data_Base* Load_DB_From_Text(char* DB_Text){
    ...
    string_Array_Buffer = Get_Attribute_Values(DB_Text,&DB->Classes[l]->Products[m]->Attributes_amount,&Char_ptr,&DB->Classes[l]->Products[m]->Extra_sub_attribute_value_amount[DB->Classes[l]->Products[m]->Attributes_amount-1]);
