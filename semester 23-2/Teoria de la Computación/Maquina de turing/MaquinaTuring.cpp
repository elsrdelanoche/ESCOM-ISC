...
struct StackAutomata {
    vector<Node> Nodes;
    StackAutomata(int n) { Nodes.resize(n + 1); }
    ...
    void addAcc(int u) { Nodes[u].acc = 1; }
    ...
    void generateGraphViz() {
        ofstream outfile("automaton.dot");
        outfile << "digraph StackAutomaton {\n";
        outfile << "  rankdir=LR;\n";
        for (int i = 0; i < Nodes.size(); i++) {
            outfile << "  " << i << " [shape=" << (Nodes[i].acc ? "doublecircle" : "circle") << "];\n";
            for (auto adj : Nodes[i].edges) {
                outfile << "  " << i << " -> " << adj.second << " [label=\"" << adj.first.outTop << "," << adj.first.inTop << "," << adj.first.dir << "\"];\n";
            }
        }
        outfile << "}\n";
        outfile.close();
        cout << "Graphviz file 'automaton.dot' generated." << endl;
    }
};
int main() {
    ...
    sa.generateGraphViz(); // Generar archivo DOT para Graphviz
    system("dot -Tpng -o automata.png automaton.dot");
    return 0;
}
